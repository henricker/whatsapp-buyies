...
export default async function usePostgresAuthState(sessionID: string, saveOnlyCreds = false) { // parametro para informar ao script para salvar apenas a key creds no DB
  ...
  const localFile = (key: string) => path.join(localFolder, (fixFileName(key) + '.json')) // função que retorna o caminho absoluto da key ja com o nome normalizado
...
const fixFileName = (file: string) => { // função que normaliza o nome da key
  if (!file) {
    return undefined;
  }
...
async function insertOrUpdateAuthKey(botId: string, keyId: string, keyJson: string) {
...
async function getAuthKey(botId: string, keyId: string) {
  const db = await getDbConnection();
...
async function deleteAuthKey(botId: string, keyId: string) {
  const db = await getDbConnection();
  if (!db) return
